...
function MoodEntryFormComponent() {
  ...
  // Handle actual submission to database
  const submitEntry = useCallback(async (entryData: any, existingEntryId?: string) => {
    let submitError
    if (existingEntryId) {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingEntry) {
        // Show confirmation dialog if entry exists
        setPendingEntryData({ data: entryData, existingId: existingEntry.id })
        setShowOverwriteDialog(true)
      } else {
        // Proceed with new entry
        await submitEntry(entryData)
      }
    } catch (error: any) {
  ...
  }, [user, isSubmitting, formDraft, supabase, toast, router, submitEntry])
  ...
  const handleConfirmOverwrite = useCallback(async () => {
    ...
    setIsSubmitting(true)
    try {
      await submitEntry(pendingEntryData.data, pendingEntryData.existingId)
    } catch (error: any) {
      console.error('Failed to save mood entry:', error)
      setError(error)
      toast({
  ...
  }, [pendingEntryData, submitEntry, toast])
      ...
      <AlertDialog open={showOverwriteDialog} onOpenChange={setShowOverwriteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You already have an entry for today. Would you like to overwrite it with your current changes?
              This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={handleCancelOverwrite}>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleConfirmOverwrite}>Confirm</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
